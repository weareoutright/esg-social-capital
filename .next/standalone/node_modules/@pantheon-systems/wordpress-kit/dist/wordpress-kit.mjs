...
import { setSurrogateKeyHeader as R } from "@pantheon-systems/cms-kit";
import { setSurrogateKeyHeader as Lt } from "@pantheon-systems/cms-kit";
...
class Wt {
  constructor(t, i) {
     ...
     */
    a(this, "options");
    this.endpoint = t, this.options = {
      ...i,
      headers: {
        "Pantheon-SKey": "1"
      },
      jsonSerializer: (i == null ? void 0 : i.method) === "GET" ? { parse: JSON.parse, stringify: JSON.stringify } : void 0
    };
  }
  /**
   ...
   */
  create() {
    return new N(this.endpoint, this.options);
  }
}
const B = "public, s-maxage=600", T = ({
  res: e,
  cacheControl: t = B
}) => {
  e.setHeader("Cache-Control", t);
}, U = ({ headers: e }) => {
  const t = e.map((o) => o.get("Surrogate-Key")).join(" ").split(" ");
...
let m = u;
var P = (m.__esModule ? m : { default: m }).default;
const O = /* @__PURE__ */ G(P), h = ({
  defaultColor: e
}) => ({
      ...
      [`&.columns-${i}`]: {
        figure: {
          [`@media (min-width: ${e})`]: {
            // This calc is to divide the width by the number of columns
            // and then subtract 1rem to account for the gap between columns/rows.
            width: `calc(${100 / i}% - 1rem) !important`
          }
        }
      }
    };
...
}, ht = c.map((e) => `.has-${e.name}-color`), bt = c.map(
  (e) => `.has-${e.name}-background-color`
), xt = c.map(
  (e) => `.has-${e.name}-border-color`
), wt = b.map(
  (e) => `.has-${e.name}-font-size`
), yt = x.map(
  (e) => `.has-${e.name}-gradient-background`
), kt = {
  content: [],
  safelist: [
    ".has-text-color",
...
class vt {
  constructor(t) {
     ...
     */
    a(this, "getColor", (t) => this.theme(
      `colors.${t.themeName}`,
      this.theme(`colors.${t.tailwindDefault}`, t.hexDefault)
    ));
    /**
     ...
     */
    a(this, "findColor", (t) => {
      const i = c.find((o) => o.themeName === t);
      if (!i)
        throw new Error(`Color ${t} not found`);
      return i;
    });
    /**
     * @private
     ...
     */
    a(this, "formatGradientColors", (t) => t.map(({ color: o, position: r }) => {
      if (o.startsWith("#"))
        return `${o} ${r}`;
      const s = this.findColor(o);
      return `${this.getColor(s)} ${r}`;
    }).join(", "));
    /**
     *
     ...
     * ```
     */
    a(this, "getColorUtilities", () => c.reduce(
      (t, i) => ({
        ...t,
        [`.has-${i.name}-color`]: {
          color: `${this.getColor(i)} !important`
        }
      }),
      {
        ".has-text-color": {
    ...
    a(this, "getBackgroundUtilities", () => {
      ...
      return c.reduce(
        (i, o) => ({
          ...i,
          [`.has-${o.name}-background-color`]: {
            backgroundColor: `${this.getColor(o)} !important`
          }
        }),
        {
          ".has-background": {
     ...
     * ```
     */
    a(this, "getBorderColorUtilities", () => c.reduce(
      (t, i) => ({
        ...t,
        [`.has-${i.name}-border-color`]: {
          borderColor: `${this.getColor(i)} !important`
        }
      }),
      {}
    ));
     ...
     * ```
     */
    a(this, "getGradientUtilities", () => x.reduce(
      (t, { name: i, direction: o, colors: r }) => ({
        ...t,
        [`.has-${i}-gradient-background`]: {
          background: `linear-gradient(${o}, ${this.formatGradientColors(
            r
          )}) !important`
        }
      }),
    ...
    a(this, "getColorByName", (t) => {
      const i = this.findColor(t);
...
class Ct {
  constructor(t) {
     ...
     * ```
     */
    a(this, "getFontSizeUtilities", () => b.reduce(
      (t, i) => ({
        ...t,
        [`.has-${i.name}-font-size`]: {
          fontSize: `${this.theme(
            `fontSize.${i.tailwind}`,
            `${i.default}`
          )} !important`
...
const g = class g {
  constructor() {
    a(this, "baseUtilities", {
      ".ps-wp-content": {
        ...g.getHeadingsAlignmentUtilities(),
        p: {
          marginRight: "auto",
          marginLeft: "auto",
          maxWidth: "720px"
...
a(g, "getHeadingsAlignmentUtilities", () => Array.from({ length: 6 }, (i, o) => o + 1).map(
  (i) => ({
    [`>h${i}`]: {
      maxWidth: "720px",
      margin: "0 auto",
...
const jt = O(function({
  ...
  const o = new vt(t), r = new Ct(t), s = new p(), d = H, w = F, y = q({
    stripeColor: t("colors.stripes", "#f2f2f2")
  }), k = I(), v = K(), C = V(), _ = Z(), S = st({
    defaultColor: o.getColorByName("primary")
  }), D = dt({
    defaultColor: o.getColorByName("primary")
  }), W = gt(), $ = mt(), j = pt(), z = ut();
  e([
    o.getBackgroundUtilities(),
    o.getBorderColorUtilities(),
    o.getColorUtilities(),
    o.getGradientUtilities(),
    r.getFontSizeUtilities(),
    r.dropCapUtilities,
    r.textAlignUtilities,
    s.baseUtilities,
    d
  ]), i([
    y,
...
export {
  Wt as GraphQLClientFactory,
  Nt as gql,
  T as setEdgeHeader,
  $t as setOutgoingHeaders,
  Lt as setSurrogateKeyHeader,
  jt as tailwindcssPlugin
};
