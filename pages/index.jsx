...
export default function Home({ pageBy, menus }) {
	...
	const { homepageHero } = editorBlocks.filter(
		(block) => block.name === 'acf/homepage-hero',
	)[0];
	const { whySocialCapitalMatters } = editorBlocks.filter(
		(block) => block.name === 'acf/why-socap',
	)[0];
	const { leadersInTheField } = editorBlocks.filter(
		(block) => block.name === 'acf/leaders-in-field',
	)[0];
	const { homepageMap } = editorBlocks.filter(
		(block) => block.name === 'acf/homepage-map',
	)[0];
...
export async function getServerSideProps({ res }) {
	...
	const headers = [postHeaders, menuHeaders];
